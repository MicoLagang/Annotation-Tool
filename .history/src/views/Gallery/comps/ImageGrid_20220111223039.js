...
function ImageGrid() {
  ...
  function updateSubmitButton() {
    console.log("Disable button kay naay annoateion wala");
  }
  ...
  function imageRender() {
    ...
    for (let index = 0; index < docs.length; index++) {
      console.log("index: " + index + "/" + "Docslenght: " + docs.length);
      console.log();
      ...
      <>
        <div
          ...
          onClick={() => acceptReject(docs[index])}
        >
          <Card
            key={docs[index].id}
            border={`${isAnnotated(docs[index]) ? "success" : "danger"}`}
            className="h-100"
            style={{
              backgroundImage: `url(${docs[index].url})`,
              backgroundRepeat: "no-repeat",
              backgroundPosition: "center",
              backgroundSize: "cover",
              borderStyle: "solid",
              borderWidth: "3px",
              border: isActive(docs[index]) ? "4px solid" : "",
            }}
          >
            <InfoOutlinedIcon
              ...
              aria-haspopup="true"
              onMouseEnter={(event) => handlePopoverOpen(event, docs[index])}
