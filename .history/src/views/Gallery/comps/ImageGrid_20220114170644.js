...
import rejectedNotificationService from "../../../services/rejectedNotification.service";
...
function ImageGrid() {
  ...
  const [disableSubmitAnnotation, setDisableSubmitAnnotation] = useState(true);
  const [totalAnnotated, setTotalAnnotated] = useState(0);
  const [showReason, setShowReason] = useState(false);
  const [imageToValidate, setImageToValidate] = useState();
  const [reason, setReason] = useState("");
  const [isAnnoationAccepted, setIsAnnotationAccepted] = useState(true);
  ...
  const currentImagesFolderName = localStorage.getItem(
    "currentImagesFolderName"
  );
  ...
  const getAnnotated = () => {
    if (annotatedImagesArray) {
      ...
      for (let index = 0; index < docs.length; index++) {
        console.log(isAnnotated(docs[index]));
        console.log(docs[index]);
        result = [...result, isAnnotated(docs[index])];
      }
    }
    return result.filter((item) => item === true).length;
  };
  ...
  //       result = [...result, isAnnotated(docs[index])];
  //     }
  //     setTotalAnnotated(result.filter((item) => item === true).length);
  //     if (result.filter((item) => item === true).length == docs.length) {
  //       setDisableSubmitAnnotation(false);
  //     }
  //   }
  // }, [docs, annotatedImagesArray]);
  ...
  function changeAcceptReject(status) {
    if (status == "accept") {
      setShowReason(false);
      setIsAnnotationAccepted(true);
    } else if (status == "reject") {
      setShowReason(true);
      setIsAnnotationAccepted(false);
    }
  }

  function updateAcceptReject() {
    teamService.acceptImageAnnotation(
      teamID,
      name,
      folderID,
      imageToValidate.id,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const data = {
          ...
          foldername: currentImagesFolderName,
  ...
  async function acceptAnnotaion(doc) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        rejectedNotificationService.create(data);
  ...
  function rejectAnnotation() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        teamService.rejectAnnotation(teamID, name, folderID, result.value);
        rejectedNotificationService.create(data);
    ...
    <>
      ...
      <Card style={styles.card}>
        ...
        <div style={styles.overlay}>
          ...
          <Typography style={styles.text}>Annotated: {getAnnotated}</Typography>
        </div>
        <div style={styles.buttons}>
          <>
            <Chip style={chip} label={status} />
            {status == "Rejected" && (
              <p>Reason: {imageFolderData.reasonStatus}</p>
            )}
          </>
        </div>
      </Card>
      ...
      {currentUserRole !== "contributor" && currentUserRole !== "validator" && (
        <div>
          {imagesURL.length > 0 ? (
            <>
      ...
      {currentUserRole === "validator" && (
        <>
          ...
          {/* <button
            ...
            data-bs-target="#acceptRejectModal"
          >
            Launch demo modal
          </button> */}
