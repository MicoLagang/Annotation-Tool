...
export default function TopNav() {
  ...
  useEffect(() => {
    if (currentUserRole == "admin") return setBgColor("#c92d39");
      ...
      return setBgColor("#834187"), showdata();
    else if (currentUserRole == "annotator")
      return setBgColor("#fcc438"), showdatarejected();
    ...
    projectFirestore
      ...
      .onSnapshot((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          ...
          setnotifications(getPostsNotifs);
        });
      });
    // handleDrawerOpenNotif(true)
  }

  function showdatarejected() {
    const getPostsNotifs = [];
    projectFirestore
      .collection("REJECTEDNOTIFICATIONS")
      .where("team", "==", teamID)
  ...
  const notification = () => (
    <List style={{ width: drawerWidth }} onClick={() => setOpenNotif(false)}>
          ...
          return (
            <>
              <ListItem>
                <Card>
                  <CardContent>
                    {item.name} submitted folder {item.foldername} date:{" "}
                    {item.date}
                  </CardContent>
                </Card>
              </ListItem>
        ...
        if (currentUserRole == "annotator") {
          return (
            <ListItem>
              <Card>
                <CardContent>
                  {item.name} {item.status} folder {item.foldername} date:{" "}
                  {item.date}
                </CardContent>
              </Card>
            </ListItem>
    ...
    <>
      <Box sx={{ flexGrow: 1 }} className="mb-4">
        ...
        <Drawer
          open={openNotif}
          anchor={"right"}
          onClose={handleDrawerCloseNotif}
        >
          {notification()}
        </Drawer>
      </Box>
